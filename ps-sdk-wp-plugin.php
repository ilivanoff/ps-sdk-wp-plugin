...
/*
  ...
  Description: Плагин включает в работу классы ps-sdk.
  Version: 1.0
  Author: ilivanoff
...
if (defined('PSSDK_PLUGIN_ENABLED')) {
    //Плагин уже подключен
} else {
    ...
    register_activation_hook(__FILE__, array($BRIDGE, 'pluginActivation'));
    register_deactivation_hook(__FILE__, array($BRIDGE, 'pluginDeactivation'));

    add_action('plugins_loaded', array($BRIDGE, 'init'));
}
